...
[Finding Optimal Sequences for Area Aggregation---\\%\hspace{0.95mm}~\\ 
...
\chaptermark{Finding Optimal Sequences for 
Area Aggregation%---\Astar vs.\ Integer Linear Programming
}
...
we want to remove these tiny areas.
...
proposed to split these tiny polygons 
...
see \textcite[\chap2]{Choudhary1992Elements}
...
\captionof{figure}{An "aggregation sequence" for 
    ...
    (see \eq\ref{eq:h_comp}), based on the number of edges. 
...
\caption{A comparison of the greedy algorithm, 
	...
	\tab\ref{tab:AreaAgg_CaseStudy1_Statistics}.
	%
	Denotations~$\sum g_\mathrm{type}$, $\sum g_\mathrm{lgth}$, 
	and~$\sum g_2$
